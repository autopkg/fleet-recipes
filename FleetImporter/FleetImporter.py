...
class FleetImporter(Processor):
    ...
    def _run_direct_upload_workflow(self):
        ...
        # POST /software/package only ever creates the first installer for a
        # title; Fleet returns 409 if the title already has one (from an
        # earlier AutoPkg run's installer, or a manual upload in the UI). Since
        # _check_existing_package() already ruled out an identical hash/filename
        # match, this is a genuinely new version that's blocked only because
        # Fleet has no "upsert" on the create endpoint. Recover by finding the
        # existing title and pushing the new version via the update endpoint
        # instead (PATCH .../software/titles/:id/package), which does version.
        if upload_info.get("package_exists"):
            existing_title_name = upload_info.get("existing_title_name")
            resolved_title_id = None
            if existing_title_name:
                resolved_title_id = self._find_software_title_id_by_name(
                    fleet_api_base, fleet_token, team_id, existing_title_name
                )

            if not resolved_title_id:
                self.output(
                    "Could not resolve the existing software title to replace its "
                    "installer. Exiting gracefully without updating Fleet."
                )
            ...
            if automatic_install:
                self.output(
                    "Warning: automatic_install has no effect when replacing the "
                    "installer of an existing software title; it only applies "
                    "when a title is first created."
                )

            self.output(
                f"Software title '{existing_title_name}' (ID: {resolved_title_id}) "
                "already has an installer. Replacing it with the new version..."
            )
            software_package = self._fleet_update_package_installer(
                fleet_api_base,
                fleet_token,
                resolved_title_id,
                pkg_path,
            ...
            if not software_package:
                self.output(
                    "Warning: Failed to replace the existing installer. "
                    "Exiting gracefully without updating Fleet."
                )
                self.env["fleet_title_id"] = resolved_title_id
                self.env["fleet_installer_id"] = None
                return
        else:
...
""".strip()
    ...
    def _fleet_upload_package(
        ...
        try:
            ...
            if e.code == 409:
                # POST only creates a title's first installer; Fleet returns 409
                # here whenever the title already has one - regardless of
                # whether this package's hash/version differs from it. Pull the
                # title name out of Fleet's error body so the caller can look up
                # the title and replace its installer instead of treating this
                # as a true no-op duplicate. Fleet's message wording depends on
                # which conflict check catches it server-side:
                #   "Couldn't add software. google-chrome-stable already has an
                #    installer available for the Workstations fleet."
                #   SoftwareInstaller "google-chrome-stable" already exists with
                #    fleet "Team 1".
                error_body = e.read().decode(errors="replace")
                self.output(
                    f"Package already exists in Fleet (409 Conflict): {error_body}"
                )
                existing_title_name = None
                # Fleet's JSON error body escapes the embedded quotes around the
                # title name (e.g. `"reason": "SoftwareInstaller \"foo\" already
                # exists..."`), so pull the already-unescaped "reason" string out
                # via json.loads before regexing it - regexing the raw body
                # directly would capture the literal backslash along with it.
                reason = error_body
                try:
                    reason = (
                        json.loads(error_body)
                        .get("errors", [{}])[0]
                        .get("reason", error_body)
                    )
                ...
                for pattern in (
                    ...
                    if match:
                        existing_title_name = match.group(1)
                        break
                return {
                    "package_exists": True,
                    "existing_title_name": existing_title_name,
                }
    ...
    def _find_software_title_id_by_name(
        self, fleet_api_base: str, fleet_token: str, team_id: int, title_name: str
    ) -> int | None:
        """Look up a software title's ID by its exact Fleet-reported name.

        Used to recover from a 409 on package creation: the hash/package_name
        checks in _check_existing_package() found no match (this is a genuinely
        new version), but Fleet rejected the create anyway because the title
        already has an installer. The software/titles endpoint has no exact-name
        filter, so we fuzzy-search with `query` and then confirm an exact match
        client-side, since the fuzzy match alone could pick the wrong title.
        """
        headers = {
            "Authorization": f"Bearer {fleet_token}",
            "Accept": "application/json",
        ...
        url = (
            f"{fleet_api_base}/api/v1/fleet/software/titles"
            f"?team_id={team_id}"
            f"&query={urllib.parse.quote(title_name)}"
            f"&available_for_install=true"
        )
        try:
            req = urllib.request.Request(url, headers=headers)
            with urllib.request.urlopen(
                ...
                if resp.getcode() == 200:
                    data = json.loads(resp.read().decode())
                    for title in data.get("software_titles", []):
            ...
            self.output(
                f"Warning: Could not look up existing software title "
                f"'{title_name}': {e}"
            )
        return None

    def _fleet_update_package_installer(
        self,
        base_url: str,
        token: str,
        title_id: int,
        ...
        """Replace/version the installer of an existing software title.

        Unlike POST /software/package (create-only, 409s if the title already
        has an installer), PATCH .../software/titles/:id/package updates it -
        this is how a new AutoPkg-built version reaches Fleet when the title
        already has a package, whether from a prior AutoPkg run or a manual
        upload in the UI. Returns a dict with title_id/installer_id/hash_sha256
        on success, or None on failure (caller falls back to a graceful skip).
        """
        url = f"{base_url}/api/v1/fleet/software/titles/{title_id}/package"
        self.output(f"Uploading replacement package to Fleet: {pkg_path}")
        ...
        try:
            ...
            error_body = e.read().decode(errors="replace")
            self.output(
                f"Warning: Failed to replace installer for title ID {title_id}: "
                f"{e.code} {error_body}"
            )
            return None
        except urllib.error.URLError as e:
            self.output(
                f"Warning: Failed to connect to Fleet API to replace installer: {e}"
            )
            return None

        if status not in (200, 201):
            self.output(
                f"Warning: Unexpected status {status} when replacing installer: "
                f"{resp_body.decode(errors='replace')}"
            )
            return None

        data = json.loads(resp_body or b"{}")
        installer = data.get("software_installer", {})
        return {
            "title_id": installer.get("title_id"),
            "installer_id": installer.get("installer_id"),
            "hash_sha256": installer.get("hash_sha256"),
        }
