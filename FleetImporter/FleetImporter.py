...
""".strip()
    ...
    def _fleet_upload_package(
        ...
        try:
            ...
            if e.code == 409:
                ...
                # title name out of Fleet's error body (e.g. `SoftwareInstaller
                # "google-chrome-stable" already exists with fleet "Team 1".`)
                # so the caller can look up the title and replace its installer
                # instead of treating this as a true no-op duplicate.
                error_body = e.read().decode(errors="replace")
                self.output(f"Package already exists in Fleet (409 Conflict): {error_body}")
                ...
                reason = error_body
                try:
                    reason = json.loads(error_body).get("errors", [{}])[0].get(
                        "reason", error_body
                    )
    ...
    def _fleet_update_package_installer(
        ...
        boundary = (
            "----FleetReplaceInstallerBoundary" + hashlib.sha1(os.urandom(16)).hexdigest()
        )
        body = io.BytesIO()
